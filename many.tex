...
{\bf Our key results appear in Eq.~(\ref{eq:posterior}), which provides the joint posterior probability distribution on the foreground and background rates and shape parameters and the provenance of individual events as either foreground or background.  Eq.~(\ref{eq:rate-shape-posterior}) is a marginalized version of Eq.~(\ref{eq:posterior}), useful when the provenance of individual events is not relevant.  In practice, these posteriors are best sampled with stochastic techniques such as Markov chain Monte Carlo.}
...
data.  At the least, we will want to know the overall {\bf dimensionless} 
...
{\bf This problem can also be avoided
...
nearly all experiments.  For example, we can include an upper limit
on the rate, $R_{\rm max}$, in the prior for $R_b$.}
\begin{widetext}
...
{\bf If the rates are estimated accurately, then a rate estimate $R_{f,1}$ above threshold $x_{\rm min}=x_1$ can be converted into a rate estimate $R_{f,2}$ above threshold $x_{\rm min}=x_2$ via 
$R_{f,1} (1-\hat{F}(x_2,\theta)) = R_{f,2} (1-\hat{F}(x_1,\theta))$; however, rate point estimates based on thresholding can have significant fluctuations, as discussed in the following section.}
...
\end{equation}
i.e.\ the data-weighted average of the Bayesian {\bf estimator} $R_{B}$
equals the prior-weighted average $R$.  Therefore all threshold values
will yield, on average, the same {\bf point} estimate of the rate.  However this
...
$N_{\rm eff}$, a {\bf property of the distribution of background events identified by filtering with this template bank}.  Moreover, the choice of
...
\begin{figure}
  \includegraphics[width=0.475\columnwidth]{sky_location}\includegraphics[width=0.475\columnwidth]{scale}
