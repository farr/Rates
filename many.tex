...
\ee
For each data set in the ensemble, compute the Bayesian estimator $R_{BE} = \int{ R p(R|data) dR}$.
Then it is immediate that 
\be 
\int{R_{BE}(data)} d(data) = \int{ R p(R) dR}\, .
\ee
\noindent I.e, the data-weighted average of the Bayesian estimate $R_{BE}$ equals the prior-weighted average $R$. \curt{Guys:  would you like me to supply the argument?}
%
...
range $[\rho - \Delta\rho/2, \rho + \Delta\rho/2]$  We write $P = P_n + P_t$, where $P_n$ is
the distribution of noise events and $P_t$ is the distribution of events from "true" astrophysical sources.   Both $P_n$ and $P_t$ are the product of a rate (of noise or true events) and the observation time.  Here we will assume that the noise distribution is Gaussian, so that $P_n$
...
\ee
For this example we will assume that the spatial distribution of true events can be approximated as uniform in Euclidean space. Then we can write the full $P$ as
%and that the spatial distribution of true events can be approximated as uniform in Euclidean %space. Then $P$ has the general form  
\be
...
For that event list, we define $\Delta R_t$ by
\be
(\Delta R_t)^2 = \int{(R - R_t)^2 p(R)\, dR }\, .
\ee
We initially set a rather low threshold, and see how $\Delta R_t$ varies as $rho_{th}$ is increased.
To find the average behavior, we do many draws of $R_t$ and corresponding data sets.

Illustrative results are displayed in Fig.~\ref{fig:delR}, where the mean value of $\Delta R_t$ is plotted as a function of $\rho_{th}$.
%$x_{th} \equiv \rho_{th}/\rho_1$.  
To create this figure, we chose $\rho_1 = 8$,
and took our prior on $R_t$ to be a uniform distribution between $1$ and $10$.   Fig.~\ref{fig:delR} also shows how the mean number of noise events on the list varies with $\rho_{th}$.  Note the relative insensitivity of  $\Delta R_t$ to $\rho_{th}$:  as the mean
number of noise events on the event list shrinks from $8$ to $\sim 10^{-6}$, 
the average $\Delta R_t$ increases only from 2.4 to 2.8.
\curt{Guys: I can produce more figs, and will produce cleaner ones.}


\begin{figure}
  \includegraphics[width=\columnwidth]{delR}
  \caption{\label{fig:delR} The meanu uncertainty in the Bayesian rate estimate
as function of the threshold value $\rho_{th}$.  Also shows how the mean number of
noise events varies with $\rho_{th}$.}
\end{figure}
