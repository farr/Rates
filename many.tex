...
\begin{abstract}
  ...
  examples of determining the rates of gravitaitonal wave events in
...
from an inhomogeneous Poisson process with differential rates
\begin{equation}
...
$\theta' = \theta \setminus \{R_{f}, R_{b} \}$.  Then $R_f$ and $R_b$
...
either 0 or 1, indicating a background or foreground event.  These
...
\begin{equation}
  \label{eq:Bayes}
  p\left(\mathset{f_i}, R_f, R_b, \theta | d \right)  \propto
  p\left( d | \mathset{f_i}, R_f, R_b, \theta\right)
  p\left(\mathset{f_i}, R_f, R_b, \theta\right). 
\end{equation}
The constant of proportionality, called the evidence, $p(d)$, is a
normalization constant independent of the state flags, rates, and
shape parameters.
...
\end{equation}
The first term is not acutally a ``prior'' in the usual sense; the
conditional probability of the $\mathset{f_i}$ on the rates is given
by
\begin{equation}
...
inhomogenous Poisson processes and the $N_f! N_b!$ differnt ways of
...
prior on rates, independent of the shape parameters
\begin{equation}
  p\left( R_f, R_b, \theta\right) = \frac{1}{\sqrt{R_f R_b}} p(\theta),
\end{equation}
...
\end{equation}
Not surprisingly, this posterior equivalent to one derived by assuming
...
\begin{equation}
  R_f = \int d^n x \, f(x, \theta)
\end{equation}
and
\begin{equation}
  R_b = \int d^n x \, b(x, \theta),  
\end{equation}
where $f$ and $b$ are rate densities on the $n$-dimensional space of
