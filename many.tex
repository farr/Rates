...
\begin{equation}
p_{\rm LE}\left( R_f, \theta | d \right) \propto \left( \frac{\hat{b}(x_N,\theta)}{2(1-\hat{B}(x_N,\theta))} + R_f \hat{f}(x_N,\theta)\right)\frac{\sqrt{\pi}}{\sqrt{1-\hat{B}(x_N,\theta)}\,\sqrt{R_f}} \exp(-R_f) .
\end{equation}
...
\end{equation}
where $\tilde{b}(x_N,\theta) =\hat{b}(x_N,\theta)/(1-\hat{B}(x_N,\theta))$. If we assume that $\tilde{b}(x_N,\theta) \ll 1$, i.e., that the loudest event is very likely to be foreground we obtain the expected result $(1- \hat{F}(x_N,\theta))\,R_f \approx 1/2$.
